...
$RFDIFFUSION_DIR/scripts/run_inference.py \
	...
	'contigmap.contigs=[C1-285/0 10-30]' \
       	'ppi.hotspot_res=[C40]' \
...
movie record size 400,400 format png supersample 4 directory $MOVIE_DIR
open $OUT_DIR/$project/*.pdb
hide all models
~disp; cartoon style protein modeh tube rad 2 sides 24; rainbow chain palette RdYlBu-5; lighting flat
...
echo -e "<img src=\"cid:animated.gif\" />" | mutt -e 'set content_type=text/html' -s "Movie" -a $MOVIE_DIR/animated.gif -a $MOVIE_DIR/session.cxs -e 'my_hdr From:RFDiffusion (RFDiffusion)' -b eugene.valkov@gmail.com -- "$USER"@nih.gov
