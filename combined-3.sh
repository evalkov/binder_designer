...
echo "\
BINDER DESIGN PIPELINE by Eugene Valkov, NCI/NIH."
...
echo "\
$num_binders binders of $binder_length residues targeting $hotspot-$hotspot_residue of `basename $model`, chain $template_chain ($template_length residues)
...
echo "\
Completed on `date "+%B %d, %Y"` at `date "+%I:%M %p"` " | tee -a $OUT_DIR/$project/settings.txt
...
hide all models
show #1 models
movie record size 1500,1500 format png supersample 4 directory /mnt/beegfs/valkov/rfdiffusion/not9_TESTING_DO_NOT_DELETE
" >> movie.cxc
...
cp binders.html top20_scoring_solutions/
cp binders.html top_scoring_solutions/
...
mv  out.sc check.point *.txt logs/
